...
- Which extra packages and apk feeds to include
...
| `rootfs_size_mb`  | number | Rootfs partition size in MiB, sent to ASU as `rootfs_size_mb` and applied by ImageBuilder as `ROOTFS_PARTSIZE`. Omit to keep the profile default (104 MiB on most targets), which is sized for NOR-flash routers and fails with "Selected packages exceed device storage" once optional sets like Docker are selected. Set it on any board that boots from SD or eMMC — the space past the squashfs becomes the persistent overlay. Must not exceed `MAX_CUSTOM_ROOTFS_SIZE_MB` in `compose.yaml` (4096). Note the `.img` is padded to this size once decompressed. |
