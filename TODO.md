...
- [ ] **Per-recipe install instructions shown with the download.** Extend the recipe schema with a free-form `install_notes` (Markdown or plain text) field that the selector renders in the downloads area next to the image link after a successful build. Device-specific advice — which partition/slot to flash, which button to hold during bring-up, "unplug ethernet on first boot so the LEDs aren't claimed," "don't plug in the SD until the blue light is steady," how to enter MASKROM mode if something goes wrong, physical orientation, etc. — belongs per-recipe because every board is different, and the user only sees the recipe they actually picked. Currently the download area is bare (just a filename and a link) and all the hard-won device quirks live in our heads and this TODO. Each recipe author captures them once and every future user of that recipe benefits.
